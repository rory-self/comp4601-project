...
#   ./run_on_board.sh mcoder          run one design
...
designs=(
  ...
  "mcoder|mcoder_interleaved|board/arith.xclbin|board/mc_host_arm||-N 4095 -n ITERS"
...
declare -A DESC=(
  ...
  [mcoder]="M-coder CABAC + interleaving (multiply-free, 0 DSP)"
...
if [ "$1" = "all" ]; then
  ...
  for d in "${designs[@]}"; do
    ...
    # hosts differ: "M sym/s" (tans, mcoder) vs "M symbols/s" (rep) -- match both.
  ...
  echo "baselines differ (bit-wise M-coder 2.8 M/s vs byte-wise tANS 56 M/s),"
