...
Design notes for `hardware/mcoder`. The measurements are in [`REPRODUCIBILITY.md`](REPRODUCIBILITY.md)
...
> Where 26,317 comes from: it is derived from the *board* measurement, not from
> cosim. `results/onfabric_result.txt` gives 31.12 M sym/s at 200 MHz, i.e.
> 200e6 / 31.12e6 = 6.427 cycles/byte, and 4095 x 6.427 = 26,318 cycles. The
> coder's own 4096 cycles is 512 bytes/lane x 8 bins/byte at II=1. Do not look
> for 26,317 in `mcoder_hls_sweep.csv` — that column is cosim, which reports
> 32,694 (ctxreg) / 34,236 (ctxram) at K=8 under an idealised AXI model.
...
**−38% LUT for +4.7% cycles**, so it is the default in `synth/hls.cfg`.
...
[`data/text_page.pgm`](data/text_page.pgm) (regenerate with
[`data/gen_text_page.py`](data/gen_text_page.py)) is bilevel "scanned text" — pure
