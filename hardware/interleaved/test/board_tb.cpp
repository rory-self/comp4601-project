/* C-sim / co-sim testbench for the board top arith_kernel(in,n,out,out_len).
 * Encodes with the kernel, decodes each of the LANES chunks, checks lossless. */
#include <stdio.h>
#include <string.h>
...
static int decode_bit(uint16_t &low, uint16_t &high, uint16_t &code, uint16_t &prob) {
    uint32_t range = (uint32_t)high - low + 1u;
    uint32_t split = (range * prob) >> PROB_BITS;
    int bit;
    if ((uint16_t)(code - low) < split) { bit = 0; high = (uint16_t)(low + split - 1); }
    else                                { bit = 1; low  = (uint16_t)(low + split); }
    for (;;) {
        if (high < HALF) {}
        else if (low >= HALF)                            { code -= HALF; low -= HALF; high -= HALF; }
        else if (low >= FIRST_QTR && high < THIRD_QTR)   { code -= FIRST_QTR; low -= FIRST_QTR; high -= FIRST_QTR; }
        else break;
        low = (uint16_t)(low << 1); high = (uint16_t)((high << 1) | 1u);
...
static int decode_chunk(const byte_t *in, int len, byte_t *out) {
    ...
    while (decode_bit(low, high, code, flag)) {
        ...
        for (int j = 7; j >= 0; j--) {
            ...
            value = (value << 1) | bit; context = (context << 1) | bit;
        }
        out[oi++] = (byte_t)value;
    }
    return oi;
...
int main() {
    ...
    unsigned r = 7;
    for (int i = 0; i < 2048; i++) { r = r * 1103515245u + 12345u; message[i] = (r >> 16) & 0xff; }
