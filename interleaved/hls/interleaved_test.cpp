...
#ifdef WIDE_INPUT
static uint64_t packed_message[(MAX_IN + 7) / 8];
#endif

static int run_case(const char *name, int n) {
#ifdef WIDE_INPUT
    memset(packed_message, 0, sizeof(packed_message));
    for (int i = 0; i < n; i++)
        packed_message[i >> 3] |= (uint64_t)message[i] << ((i & 7) << 3);
    int encoded_len =
        arith_encode_interleaved(packed_message, n, encoded);
#else
